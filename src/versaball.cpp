...
namespace versaball
{
                                 ...
                                 std_srvs::Empty::Response &res)
    {
        ...
        while(remaining_actions > 0)
        {
            ...
            for (int i=0; i<4; i++)
            {
                _do_action(remaining_actions, start_time, now, versaball_actions[i], true);
                _do_action(remaining_actions, start_time, now, versaball_actions[i], false);
            }
            ROS_INFO_STREAM("Remaining actions to do "<<remaining_actions);
    ...
    void VersaballNode::_do_action(unsigned short& remaining_actions,
        ros::Time start_time, ros::Time::now, action_t current_action,
        bool state)
    {
        ros::Duration delay = state?current_action.delay_start:current_action.delay_stop;
        std::string description = (state?"enable":"disable") + " "
    ...
    {
        _start_pressure			= ros::Duration(config.start_pressure*e-3);
        _open_presure_valve		= ros::Duration(config.open_presure_valve*e-3);
        _close_pressure_valve	= ros::Duration(config.close_pressure_valve*e-3);
        _stop_pressure			= ros::Duration(config.stop_pressure*e-3);
        _start_void				= ros::Duration(config.start_void*e-3);
        _open_void_valve		= ros::Duration(config.open_void_valve*e-3);
        _close_void_valve		= ros::Duration(config.close_void_valve*e-3);
        _stop_void				= ros::Duration(config.stop_void*e-3);
    }
